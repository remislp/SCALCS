...
#        rcj.printout(self.mec, jpar, output=self.log)
    ...
    def onPlotCJumpRiseVConc(self):
        ...
        if dialog.exec_():
            cmin, cmax, self.length = dialog.return_par()

        self.parent.txtPltBox.clear()
        str = ('===== WEIGHTED ON/OFF TAU VERSUS CONCENTRATION =====\n' +
            'Pulse width = {0:.5g} millisec\n'.format(self.length * 1000) +
...
class CJumpParDlg(QDialog):
    ...
    def __init__(self, parent=None, profile='rcj', cjlen=0.05, cjstep=5e-6, cjargs=None):
        ...
        if cjargs:
            ...
            if self.profile == 'rcj':
                ...
                self.length = cjargs[3] * 1000 # Pulse halfwidth in ms.
                ...
                self.prepulse = cjargs[2] * 1000 # Time before pulse starts (ms)
                self.length = cjargs[3] * 1000 # Pulse halfwidth in ms. 
                #TODO: this is temporary fix of a bug in reading parameters
                if len(cjargs) < 5:
                    self.inter = 10.0
                else:
            ...
            self.rise = 200 # 10-90% rise time for error function in microsec.
            self.decay = 200 # 90-10% decay time for error function in microsec.
            self.length = 10 # Pulse halfwidth in ms.
            # 'instexp' profile
        ...
        self.concEdit = QLineEdit(str(self.cmax))
        self.concEdit.setMaxLength(12)
        self.concEdit.editingFinished.connect(self.on_par_changed)
        ...
        if self.profile == 'rcj':
            ...
            self.widthEdit = QLineEdit(str(self.length))
            ...
            self.centreEdit = QLineEdit(str(self.centre))
            self.centreEdit.setMaxLength(12)
            self.centreEdit.editingFinished.connect(self.on_par_changed)
            layout.addWidget(self.centreEdit)
            layoutMain.addLayout(layout)

            layout = QHBoxLayout()
            layout.addWidget(QLabel("Pulse 10-90% rise time (microsec):"))
            self.riseEdit = QLineEdit(str(self.rise))
            self.riseEdit.setMaxLength(12)
            self.riseEdit.editingFinished.connect(self.on_par_changed)
            layout.addWidget(self.riseEdit)
            layoutMain.addLayout(layout)

            layout = QHBoxLayout()
            layout.addWidget(QLabel("Pulse 90-10% decay time (microsec):"))
            self.decayEdit = QLineEdit(str(self.decay))
            self.decayEdit.setMaxLength(12)
            self.decayEdit.editingFinished.connect(self.on_par_changed)
            ...
            self.prepulseEdit = QLineEdit(str(self.prepulse))
            self.prepulseEdit.setMaxLength(12)
            self.prepulseEdit.editingFinished.connect(self.on_par_changed)
            layout.addWidget(self.prepulseEdit)
            layoutMain.addLayout(layout)

            layout = QHBoxLayout()
            layout.addWidget(QLabel("Decay time constant (ms):"))
            self.decayEdit = QLineEdit(str(self.tdec))
            self.decayEdit.setMaxLength(12)
            self.decayEdit.editingFinished.connect(self.on_par_changed)
            layout.addWidget(self.decayEdit)
            layoutMain.addLayout(layout)
            ...
            self.interEdit = QLineEdit(str(self.inter))
            self.interEdit.setMaxLength(12)
            self.interEdit.editingFinished.connect(self.on_par_changed)
            layout.addWidget(self.interEdit)
            layoutMain.addLayout(layout)

        layout = QHBoxLayout()
        layout.addWidget(QLabel("Record length (millisec):"))
        self.reclengthEdit = QLineEdit(str(self.reclength))
        self.reclengthEdit.setMaxLength(12)
        self.reclengthEdit.editingFinished.connect(self.on_par_changed)
        layout.addWidget(self.reclengthEdit)
        layoutMain.addLayout(layout)

        layout = QHBoxLayout()
        layout.addWidget(QLabel("Sampling interval (microsec):"))
        self.stepEdit = QLineEdit(str(self.step))
        self.stepEdit.setMaxLength(12)
        self.stepEdit.editingFinished.connect(self.on_par_changed)
        layout.addWidget(self.stepEdit)
        layoutMain.addLayout(layout)

        layout = QHBoxLayout()
        layout.addWidget(QLabel("Background concentration (mM):"))
        self.bckgrconcEdit = QLineEdit(str(self.cb))
        self.bckgrconcEdit.setMaxLength(12)
        self.bckgrconcEdit.editingFinished.connect(self.on_par_changed)
    ...
    def on_par_changed(self):
        ...
        if self.profile == 'rcj':
            ...
            self.length = float(self.widthEdit.text()) * 0.001
        elif self.profile == 'instexp':
            ...
            self.prepulse = float(self.prepulseEdit.text()) * 0.001
            self.length = float(self.widthEdit.text()) * 0.001
            self.inter = float(self.interEdit.text()) / 1000.0
    ...
    def return_par(self):
        ...
        if self.profile == 'rcj':
            cargs = (self.cmax, self.cb, self.centre, self.length,
            ...
            cargs = (self.cmax, self.cb, self.prepulse, self.length)
            cfunc = cjumps.pulse_square
        elif self.profile == 'square2':
            cargs = (self.cmax, self.cb, self.prepulse, self.length, self.inter)
...
class CJumpParDlg2(QDialog):
    ...
    def __init__(self, parent=None, width = 0.01, cmin=1e-6, cmax=0.001):
        ...
        self.length = width * 1000 # Pulse halfwidth in ms.
    ...
    def return_par(self):
        ...
        Return parameter dictionary on exit.
        """
        return self.cmin, self.cmax, self.length
